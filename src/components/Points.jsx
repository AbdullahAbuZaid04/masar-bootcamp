        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <form action={deletePoint}>
              ...
              <button type="submit" title="Delete Point" className="text-red-500 hover:bg-red-500 hover:text-white p-2 rounded-full cursor-pointer transition-all duration-200">
                <Trash size={18} />
              </button>
            </form>
          </div>
          <div className="space-y-3">
            <div className={`flex items-center gap-3 text-gray-500 ${point.status?.toLowerCase() === 'available' ? 'text-green-700' : 'text-red-700'}`}>
...

      {
        (!points || points.length === 0) && (
          <div className="text-center py-20 bg-white rounded-xl border-2 border-dashed">
            <p className="text-gray-400">No distribution points found.</p>
          </div>
        )
      }
    </div>
